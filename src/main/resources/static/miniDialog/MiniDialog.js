...
!(( global, factory ) => {
	...
	const Icon = type => {
     ...
     ///.mini-dialog-footer    里拷贝出来height:30px;
	// 对话框样式 .mini-dialog-wrapper-show{animation:MiniDialogWrapperShow .35s;animation-fill-mode:forwards} .mini-dialog-container,.mini-dialog-container *{box-sizing: content-box!important}剪切出来的源码
	const DialogCSS = `
		...
		.mini-dialog-mask{position:absolute;top:0;left:0;width:100%;height:100%;background:rgba(67, 87, 121,.6);pointer-events:auto}
		...
		.mini-dialog-main{position:relative;padding:15px 0;min-height:80px;font-size:14px;line-height:500%;overflow:auto;word-break:break-all;color:#292929;-webkit-overflow-scrolling:touch;overscroll-behavior:contain}
		...
		.mini-dialog-prompt input:focus,.mini-dialog-prompt textarea:focus{border-color:#6681cc}
		...
		.mini-dialog-footer{position:relative;text-align:right;padding:15px 0;border-top:#e6e6e6 solid 1px}
		.mini-dialog-footer>div{display:inline-block;font-size:14px;cursor:pointer;text-align:center;border:#e8e8e8 solid 1px;height:32px;line-height:30px;vertical-align:middle;border-radius:4px;padding:0 14px;transition:.2s}
		div.mini-dialog-ok{color:#fff;background:#6681cc;border-color:#6681cc;margin:0 15px 0 5px}
		...
		div.mini-dialog-checkbox.active div{background:#6681cc;border-color:#6681cc}
		...
		.mini-dialog-autoclose{position:absolute;left:0;bottom:0;height:3px;background:#6681cc;width:100%;transform:scaleX(0);transform-origin:left center;transition-property:transform;transition-timing-function:linear}
	...
	const Dialog = ( ...arg ) => {
		...
		if ( Fn.isPlainObject( opt.iframeContent ) ) {
			let {id, src, height } = iframeContent;
			if ( !src || !height ) {
				return;
			}
			...
			$main.innerHTML = `<iframe id="${id}" src="${ src }" frameborder="0" scrolling="auto"></iframe>`;
		}
 ...
 * 重置确定按钮
 * ***/
function dialog_ResetBtn(){
	$(".mini-dialog-ok").removeAttr("style");
	$(".mini-dialog-ok").html("<span>确定</span>");
}
 ...
 * 重置确定按钮
 * ***/
function dialog_LoadingBtn(){
	$(".mini-dialog-ok").css({"opacity":"0.5","pointer-events":"none"});  
	$(".mini-dialog-ok").html("<i></i><span>提交中</span>");
}
